 ...
 * $imported_mem_read call=mem_read_calltf size=64 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_mem_read vpiSysFuncSized 64 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 mem_read_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = mem_read(arg_1, arg_2, arg_3);
...
/* sft: $imported_mem_read vpiSysFuncSized 64 unsigned */

/* tab: $imported_mem_read call=mem_read_calltf size=64 acc=rw:%TASK */

PLI_INT32 mem_read_sizetf(PLI_BYTE8 *user_data)
{
  return 64u;
}
